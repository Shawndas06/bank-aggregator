"""
Сервис для работы со счетами (обновлено для реального API)
"""
import logging
import json
...
class AccountService:
    ...
    def create_account(
        ...
        """
        Создаёт новый счёт через реальное API банка
        
        Шаги:
        1. Генерируем client_id (например: team200-1)
        2. Получаем токен банка
        3. Создаём consent
        4. Получаем счета из банка
        5. Сохраняем первый счёт в БД
        """
        try:
            # Генерируем client_id для пользователя
            client_id = f"{settings.TEAM_CLIENT_ID}-{user_id}"
            
            # Создаём consent для доступа к счетам
    ...
    def attach_account(
        ...
        """Привязывает счёт к пользователю"""
    ...
    def get_account_info(
        ...
        """Получает информацию о счёте с кешированием"""
    ...
    def get_account_balance(
        ...
        """Получает баланс счёта с кешированием"""
        ...
        # Получаем из реального API банка
        client_id = f"{settings.TEAM_CLIENT_ID}-{user_id}"
    ...
    def get_account_transactions(
        ...
        """Получает транзакции по счёту с кешированием"""
